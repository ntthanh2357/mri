...
export const deleteUser = async (req, res) => {
  try {
    ...
    if (user.role === "admin" && user.email === "admin@neuroscan.com") {
      res.status(403).json({ success: false, message: "Không thể xóa tài khoản Quản trị viên hệ thống cấp cao." });
      return;
    }
    await User.findByIdAndDelete(id);
    authCache.invalidateUser(id);
    res.status(200).json({ success: true, message: `Đã xóa người dùng ${user.email} thành công.` });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
export const verifyTenantIsolation = async (req, res) => {
  try {
    ...
    const allVisits = await Visit.find({ hospitalId: { $in: hospitals.map(h => h._id) } })
      .select("hospitalId doctorId nurseId technicianId")
      .lean();

    const visitsByHospital = {};
    allVisits.forEach(v => {
      const hIdStr = v.hospitalId.toString();
      if (!visitsByHospital[hIdStr]) visitsByHospital[hIdStr] = [];
      visitsByHospital[hIdStr].push(v);
    });

    for (const h of hospitals) {
      const visits = visitsByHospital[h._id.toString()] || [];
...
export const backupHospitalData = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: `Sao lưu dữ liệu bệnh viện ${hospital.name} thành công an toàn.`,
      fileName,
      downloadUrl: `/admin/hospitals/${id}/backup/download/${fileName}`
    });
  } catch (error) {
    res.status(500).json({ success: false, message: "Lỗi máy chủ: " + error.message });
  }
...
export const downloadHospitalBackup = async (req, res) => {
  try {
    ...
    if (safeName !== fileName) {
      return res.status(400).json({ success: false, message: "Tên file không hợp lệ hoặc chứa ký tự nguy hiểm." });
    }
    ...
    if (backupData.hospital?._id?.toString() !== id) {
      return res.status(403).json({ success: false, message: "Bạn không có quyền truy cập tệp sao lưu của cơ sở khác." });
    }

    res.download(filePath, safeName);
  } catch (error) {
    res.status(500).json({ success: false, message: "Lỗi tải tệp sao lưu: " + error.message });
  }
};
...
export const restoreHospitalData = async (req, res) => {
  try {
    ...
    if (Array.isArray(backupData.users) && backupData.users.length > 0) {
      ...
      const emailsToRestore = backupData.users.map(u => u.email).filter(Boolean);
      const existingUsers = await User.find({ email: { $in: emailsToRestore } }).lean();
      const existingUsersMap = {};
      existingUsers.forEach(u => existingUsersMap[u.email] = u);

      for (const u of backupData.users) {
        if (!u.email) continue;
        const existing = existingUsersMap[u.email];
        if (existing) {
...
// ─── K.6 — Quản lý subscription & giới hạn bệnh viện ────────────────────────
// @route GET /admin/subscription-dashboard
...
export const getSubscriptionDashboard = async (req, res) => {
  try {
    ...
    if (!hospital) return res.status(404).json({ success: false, message: "Không tìm thấy bệnh viện." });

    const now = new Date();
    const expiresAt = hospital.subscriptionExpiresAt ? new Date(hospital.subscriptionExpiresAt) : null;
    const daysUntilExpiry = expiresAt ? Math.ceil((expiresAt - now) / (1000 * 60 * 60 * 24)) : null;

    // Đếm số bệnh nhân hiện tại
    ...
    const usagePercent = Math.round((currentPatientCount / maxPatients) * 100);

    // Cảnh báo
    const warnings = [];
    if (daysUntilExpiry !== null && daysUntilExpiry <= 7) {
      warnings.push(`⚠️ Gói ${hospital.subscriptionPlan} sắp hết hạn sau ${daysUntilExpiry} ngày (${expiresAt?.toLocaleDateString("vi-VN")}).`);
    }
    if (usagePercent >= 90) {
      warnings.push(`⚠️ Đã sử dụng ${usagePercent}% giới hạn bệnh nhân (${currentPatientCount}/${maxPatients}). Cần nâng cấp gói.`);
    }
    ...
    res.status(200).json({
      ...
      usage: {
        ...
        maxPatients,
        usagePercent,
        remainingSlots: Math.max(0, maxPatients - currentPatientCount),
      },
      warnings,
      features: {
        trial: ["Tối đa 50 bệnh nhân", "1 phòng MRI", "AI cơ bản"],
        basic: ["Tối đa 200 bệnh nhân", "3 phòng MRI", "AI đầy đủ", "Báo cáo vận hành"],
        pro: ["Không giới hạn bệnh nhân", "Không giới hạn phòng MRI", "AI nâng cao + 3D", "BHYT", "Hỗ trợ ưu tiên"],
...
// ─── K.6 — Nâng cấp subscription (Super Admin only) ─────────────────────────
// @route PUT /admin/hospitals/:hospitalId/upgrade-subscription
// @access Private (admin)
export const upgradeHospitalSubscription = async (req, res) => {
  try {
    if (req.user.role !== "admin") {
      return res.status(403).json({ success: false, message: "Chỉ Super Admin mới được thay đổi gói đăng ký." });
    }

    const { hospitalId } = req.params;
    const { subscriptionPlan, expiresInDays = 365, maxPatients } = req.body;

    if (!["trial", "basic", "pro"].includes(subscriptionPlan)) {
      return res.status(400).json({ success: false, message: "Gói không hợp lệ (trial/basic/pro)." });
    }
    ...
    if (maxPatients) updates["pricing.maxPatients"] = Number(maxPatients);

    const hospital = await Hospital.findByIdAndUpdate(hospitalId, updates, { new: true });
    if (!hospital) return res.status(404).json({ success: false, message: "Không tìm thấy bệnh viện." });

    await AuditLog.create({
      action: "K6_UPDATE_SUBSCRIPTION",
      performedBy: req.user.id,
      targetId: hospitalId,
      targetModel: "Hospital",
      details: `Cập nhật gói ${subscriptionPlan}, hết hạn ${expiresAt.toLocaleDateString("vi-VN")}`,
    });

    res.status(200).json({
      success: true,
      message: `Đã cập nhật gói ${subscriptionPlan} cho bệnh viện.`,
      hospital: {
        id: hospital._id,
        name: hospital.name,
        subscriptionPlan: hospital.subscriptionPlan,
        subscriptionExpiresAt: hospital.subscriptionExpiresAt,
        maxPatients: hospital.pricing?.maxPatients,
      },
    });
  } catch (error) {
    res.status(500).json({ success: false, message: "Lỗi máy chủ: " + error.message });
