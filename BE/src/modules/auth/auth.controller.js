...
export const register = async (req, res) => {
  try {
    ...
    if (req.user && req.user.role === "hospital_admin") {
      const staffOnlyRoles = ["doctor", "nurse", "technician", "receptionist"];
      if (!staffOnlyRoles.includes(role)) {
        res.status(403).json({ message: "Quản trị bệnh viện chỉ được tạo tài khoản bác sĩ, điều dưỡng, kỹ thuật viên, lễ tân." });
        return;
      }
    ...
    // Môi trường dev hoặc demo: tự động kích hoạt bệnh nhân để đăng nhập ngay mà không bị nghẽn OTP
    const isAutoVerified = isPatient && process.env.NODE_ENV !== "production";
    ...
    // Send OTP email if patient and not auto-verified
    let emailSent = false;
    if (role === "patient" && !isAutoVerified) {
      try {
    ...
    res.status(201).json({
      message: (role === "patient" && !isAutoVerified)
      ...
      requiresVerification: role === "patient" && !isAutoVerified,
...
export const login = async (req, res) => {
  try {
    ...
    // Reject login if account is locked permanently
    if (user.isLocked) {
      res.status(403).json({ message: "Tài khoản của bạn đã bị khóa. Vui lòng liên hệ quản trị viên." });
      return;
    }

    // Reject login if account is temporarily locked due to brute-force attempts
    if (user.lockUntil && new Date(user.lockUntil) > new Date()) {
      const minutesLeft = Math.ceil((new Date(user.lockUntil) - new Date()) / 60000);
      res.status(403).json({
        message: `Tài khoản tạm thời bị khóa do nhập sai mật khẩu quá 5 lần liên tiếp. Vui lòng thử lại sau ${minutesLeft} phút để bảo vệ thông tin.`,
      });
      return;
    }
    ...
    if (!isMatch) {
      ...
      let updateFields = { failedLoginAttempts: failedAttempts };

      if (failedAttempts >= 5) {
        updateFields.lockUntil = new Date(Date.now() + 15 * 60 * 1000); // 15 phút
        ...
        try {
          await AuditLog.create({
            ...
            details: `CẢNH BÁO AN NINH: Tài khoản ${email} bị tạm khóa 15 phút do nhập sai mật khẩu 5 lần liên tiếp.`,
          });
        } catch { /* audit log non-blocking */ }

        await User.findByIdAndUpdate(user._id, updateFields);
        res.status(403).json({
          message: "Tài khoản của bạn đã tạm thời bị khóa 15 phút do nhập sai mật khẩu quá 5 lần liên tiếp nhằm bảo vệ an toàn thông tin.",
        });
        return;
      }
      ...
      await User.findByIdAndUpdate(user._id, updateFields);
      res.status(400).json({
        message: `Email hoặc mật khẩu không chính xác. Bạn còn ${5 - failedAttempts} lần thử trước khi tài khoản bị khóa tạm thời.`,
      });
      return;
    }
...
export const changePassword = async (req, res) => {
  try {
    ...
    await user.save();
    authCache.invalidateUser(user._id.toString());
