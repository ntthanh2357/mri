...
export default function App() {
  ...
  const [walletBalance, setWalletBalance] = useState(2450000); // in VNĐ
  const [verifiedCount, setVerifiedCount] = useState(3); // Matches backoffice verified docs count
  const [totalDatasetSales, setTotalDatasetSales] = useState(120000000); // VNĐ
  const [datasetsCount, setDatasetsCount] = useState(4);
  ...
  // Neuroscan Specific Durable States
  const [hardMiningCount, setHardMiningCount] = useState(12);
  const [patientsPremium, setPatientsPremium] = useState(['PT-8821']); // Nguyễn Văn An has premium
  ...
  // Log append helper
  const addSystemLog = (action, moduleName, details) => {
    const newLog = {
  ...
  const handleEditPatient = (edited) => {
  ...
  const handleToggleDocStatus = (id, newStats) => {
