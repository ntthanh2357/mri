...
import { Patient, Appointment, Prescription, Invoice } from '@neuroscan/types';
...
interface PatientsViewProps {
  ...
  onSetEditingPatient: (patient: Patient | null) => void;
}
...
export default function PatientsView({
  ...
  const [selectedPatientEHR, setSelectedPatientEHR] = useState<Patient | null>(null);
  const [activeEHRSubTab, setActiveEHRSubTab] = useState<'info' | 'appointments' | 'prescriptions' | 'billing'>('info');
  ...
  return (
    <div className="space-y-6 relative">
      ...
      {selectedPatientEHR && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-3xs z-50 flex justify-end animate-fade-in">
                  ...
                  <div className="bg-white rounded-xl border border-slate-150 p-4 shadow-3xs grid grid-cols-1 sm:grid-cols-2 gap-3.5 text-xs text-slate-600">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <span className="font-bold">{selectedPatientEHR.status === 'Active' ? 'Đang điều trị nội bộ' : 'Tạm thời kết thúc hồ sơ'}</span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-3.5">
                    ...
                    {['Nam', 'Nữ'].map(g => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            if (editingPatient) onSetEditingPatient({...editingPatient, gender: g as 'Nam' | 'Nữ'});
                            else setNewPatient({...newPatient, gender: g as 'Nam' | 'Nữ'});
                          }}
                ...
                <div className="space-y-1">
                  ...
                  <select 
                    ...
                    onChange={(e) => {
                      if (editingPatient) onSetEditingPatient({...editingPatient, status: e.target.value as 'Active' | 'Inactive'});
                      else setNewPatient({...newPatient, status: e.target.value as 'Active' | 'Inactive'});
                    }}
                    className="w-full border border-slate-200 rounded-xl px-3 py-2 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-teal-500/20 text-xs sm:text-sm font-bold"
                  >
