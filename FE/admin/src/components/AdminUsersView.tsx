...
import { Patient, Doctor } from '@neuroscan/types';
...
export default function AdminUsersView({ patients, doctors, addSystemLog }: AdminUsersViewProps) {
  ...
  const handleLockUser = (userId: string) => {
    setUsersList(prev => 
      prev.map(u => {
        if (u.id === userId) {
          addSystemLog('Khóa tài khoản', 'System', `Đã khóa truy cập của người dùng ${u.name} (ID: ${u.id}) do vi phạm chính sách.`);
  ...
  const handleUnlockUser = (userId: string) => {
    setUsersList(prev => 
      prev.map(u => {
        if (u.id === userId) {
          addSystemLog('Mở khóa tài khoản', 'System', `Đã phục hồi quyền đăng nhập cho người dùng ${u.name} (ID: ${u.id}).`);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-white border border-slate-150 rounded-2xl p-5 shadow-3xs">
          <h3 className="text-sm font-bold text-slate-800 mb-4 font-sans">Danh Sách Người Dùng Hệ Thống</h3>
                      ...
                      <td className="py-3.5 px-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            ...
                            <span className="text-[9px] text-slate-400 font-mono block">IP: {user.ipAddress}</span>
                          </div>
                        </div>
                      </td>
          ...
          {selectedUser ? (
            <div className="space-y-5" id="user-details-pane">
              ...
              <div className="flex items-center gap-3 bg-slate-50/60 p-3 rounded-xl border border-slate-100">
                ...
                />
                <div>
                  <h4 className="text-sm font-black text-slate-850 leading-tight">{selectedUser.name}</h4>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mt-0.5">{selectedUser.role}</p>
                </div>
              </div>
            ...
            <div className="h-64 flex flex-col items-center justify-center border border-dashed border-slate-200 rounded-xl bg-slate-50 text-center p-4">
              <User className="w-10 h-10 text-slate-350 stroke-[1.5] mb-2" />
              <p className="text-xs font-bold text-slate-500">Chưa Chọn Tài Khoản</p>
