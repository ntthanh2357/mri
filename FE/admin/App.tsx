...
import {
  ...
  Invoice,
  SystemLog
} from '@neuroscan/types';

import AdminLoginView from './src/components/AdminLoginView';
...
export default function App() {
  ...
  if (!isAdminLoggedIn) {
    return <AdminLoginView onLoginSuccess={() => setIsAdminLoggedIn(true)} />;
  }
      ...
      <aside className={`bg-[#0F172A] text-[#9ca3af] flex flex-col shrink-0 border-r border-[#1e293b]/50 select-none z-40 transition-all duration-300 ease-in-out h-screen overflow-hidden ${
        ...
        <div className={`border-b border-[#1e293b]/40 flex items-center justify-between transition-all duration-300 ${
          ...
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-9 h-9 bg-gradient-to-tr from-blue-600 to-emerald-550 rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/20 shrink-0">
        ...
        <div className="mt-auto pt-4 border-t border-white/10 px-4 pb-6 shrink-0">
          <button 
            ...
            className={`flex items-center gap-3 rounded-[12px] text-xs font-semibold cursor-pointer text-[#9ca3af] hover:bg-rose-950/40 hover:text-rose-450 transition-all duration-250 ease-out hover:translate-x-[2px] ${
              sidebarCollapsed ? 'justify-center px-0 w-11 h-11 mx-auto' : 'px-4 w-full'
            }`}
            style={{ height: '44px', transition: 'all 0.25s ease' }}
