...
// DTO from API — different from legacy SystemLog mock type
...
export default function AdminAuditLogsView() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiRequest<{ success: boolean; logs: AuditLogEntry[] }>(
        '/admin/audit-logs',
      );
      setLogs(data.logs ?? []);
    } catch (err: any) {
  ...
  const handleAnonymize = async () => {
    ...
    try {
      await apiRequest<{ success: boolean; modifiedCount: number }>('/admin/anonymize', {
        method: 'POST',
      });
